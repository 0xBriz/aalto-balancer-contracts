...
describe("Mainnet setup", () => {
  ...
  async function logWeights(time) {
    for (const gAdd of gaugeAddresses) {
      console.log(gAdd);
      const w = await gaugeController.gauge_relative_weight(gAdd, time);
      console.log(formatEther(w));

      const straightWeight = await gaugeController.get_gauge_weight(gAdd);
      console.log(formatEther(straightWeight));
    }
  }

  it("should not end us", async () => {
    await logWeights(moment().add(1, "week").unix());
    const amesGauge = "0xDA9D57cAabBe48301d71b7eEaf546B4A206118d3";
    const wrapGauge = "0xD105D7e65922e1ac725b799481c5c113cFfA2f0D";

    // await gaugeController.change_gauge_weight(amesGauge, parseEther("60000"));
    ...
    await gaugeController.change_gauge_weight(veGauge, parseUnits("180000"));
    await gaugeController.change_gauge_weight(stablesGauge, parseUnits("1"));
    await gaugeController.change_gauge_weight(ashareBusdGaugeAddress, parseUnits("5"));
    await gaugeController.change_gauge_weight(wbnbBusdGauge, parseUnits("2"));
    await gaugeController.change_gauge_weight(triBlueChipsGuage, parseUnits("4"));
    await gaugeController.change_gauge_weight(wbnbBusdGauge, parseUnits("12"));
    await gaugeController.change_gauge_weight(usdcBnb, parseUnits("3"));
    await gaugeController.change_gauge_weight(technicolorGauge, parseUnits("19"));
    await gaugeController.change_gauge_weight(blueMaxiGauge, parseUnits("19"));

    // await gaugeController.change_gauge_weight(amesGauge, parseUnits("16"));

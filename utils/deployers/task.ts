...
export default class Task {
  ...
  async verify(
    ...
    try {
      if (!this._verifier)
        return logger.warn("Skipping contract verification, no verifier defined");
  ...
  async check(name: string, args: Array<Param> = [], libs?: Libraries): Promise<Contract> {
    ...
    if (deploymentTx.data === expectedDeploymentTxData) {
      logger.success(
        `Verified contract '${name}' on network '${this.network}' of task '${this.id}'`
      );
    } else {
      throw Error(
  ...
  dir(): string {
    ...
    throw Error(
      `Could not find a directory at ${nonDeprecatedDir}, ${deprecatedDir} or ${scriptsDir}`
    );
  }
  ...
  artifact(contractName: string, fileName?: string): Artifact {
    ...
    const builds: {
      ...
      : this.buildInfos().reduce(
          (result, info: BuildInfo) => ({ ...result, ...info.output.contracts }),
          {}
        );
    ...
    return getArtifactFromContractOutput(
      sourceName,
      contractName,
      builds[sourceName][contractName]
    );
  }

...
export async function checkActionIds(task: Task): Promise<void> {
  ...
  for (const [contractName, actionIdData] of Object.entries(taskActionIdData)) {
    ...
    if (adaptorUsageMatch && actionIdsMatch) {
      logger.success(
        `Verified recorded action IDs of contract '${contractName}' of task '${task.id}'`
      );
    } else {
      throw Error(
...
export function checkActionIdUniqueness(network: string): void {
  ...
  if (
    JSON.stringify(duplicateActionIdsMapping) === JSON.stringify(expectedDuplicateActionIdsMapping)
  ) {
    ...
    for (const [actionId, instances] of Object.entries(duplicateActionIdsMapping)) {
      if (
        JSON.stringify(instances) === JSON.stringify(expectedDuplicateActionIdsMapping[actionId])
      ) {
...
export async function getActionIds(
  ...
  const { ignoredFunctions } = safeReadJsonFile<string[]>(
...
async function getActionIdSource(
  ...
  if (contractIsAuthorizerAware) {
    if (factoryOutput) {
      ...
      return {
        useAdaptor: false,
        actionIdSource: await task.instanceAt(contractName, factoryOutput),
      };
    } else {
    ...
    return {
      useAdaptor: true,
      actionIdSource: await adaptorTask.deployedInstance("AuthorizerAdaptor"),
    };
  }
}
  ...
  const actionIdsMapping: Record<string, ActionIdInfo[]> = Object.entries(actionIdFileContents)
    .flatMap(([taskId, taskData]) =>
      Object.entries(taskData).flatMap(([contractName, contractData]) =>
        Object.entries(contractData.actionIds).map<[string, ActionIdInfo]>(
          ([signature, actionId]) => [
            actionId,

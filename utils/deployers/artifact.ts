...
export function checkArtifact(task: Task): void {
  ...
  if (existsSync(buildInfoDirectory) && statSync(buildInfoDirectory).isDirectory()) {
    for (const buildInfoFileName of readdirSync(buildInfoDirectory)) {
      ...
      if (artifactMatch) {
        logger.success(
          `Verified artifact integrity of contract '${contractName}' of task '${task.id}'`
        );
      } else {
        throw Error(
...
function readContractArtifact(task: Task, contractName: string): Artifact | null {
  ...
  const artifact = artifactFileExists
    ? JSON.parse(readFileSync(artifactFilePath).toString())
    : null;
...
export function getArtifactFromContractOutput(
  ...
  let deployedBytecode: string =
    evmDeployedBytecode && evmDeployedBytecode.object ? evmDeployedBytecode.object : "";

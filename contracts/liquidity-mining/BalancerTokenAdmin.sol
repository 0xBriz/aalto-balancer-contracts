...
contract BalancerTokenAdmin is IBalancerTokenAdmin, SingletonAuthentication, ReentrancyGuard {
    ...
    //uint256 public constant override RATE_REDUCTION_COEFFICIENT = 1; // 1%
    uint256 public constant override RATE_DENOMINATOR = 1e18;
     ...
     */
    function activate(address initialMintReceiver) external override nonReentrant authenticate {
        ...
        require(
            _balancerToken.hasRole(adminRole, address(this)),
            "BalancerTokenAdmin is not an admin"
        );
        ...
        require(
            _balancerToken.getRoleMemberCount(adminRole) == 0,
            "Address exists with admin rights"
        );
        require(
            _balancerToken.hasRole(minterRole, address(this)),
            "BalancerTokenAdmin is not a minter"
        );
        require(
            _balancerToken.hasRole(snapshotRole, address(this)),
            "BalancerTokenAdmin is not a snapshotter"
        );
        require(_balancerToken.getRoleMemberCount(minterRole) == 1, "Multiple minters exist");
        require(
            _balancerToken.getRoleMemberCount(snapshotRole) == 1,
    ...
    function updateMiningParameters() external {
        require(
            block.timestamp >= _startEpochTime.add(RATE_REDUCTION_TIME),
            "Epoch has not finished yet"
        );
        _updateMiningParameters();
    }

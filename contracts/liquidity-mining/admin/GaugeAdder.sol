...
contract GaugeAdder is IGaugeAdder, SingletonAuthentication, ReentrancyGuard {
     ...
     */
    function getFactoryForGaugeTypeCount(GaugeType gaugeType)
        external
        view
        override
        returns (uint256)
     ...
     */
    function addGaugeFactory(ILiquidityGaugeFactory factory, GaugeType gaugeType)
        external
        override
        authenticate
    {
        ...
        require(
            uint256(gaugeType) < uint256(_gaugeController.n_gauge_types()),
            "Invalid gauge type"
        );

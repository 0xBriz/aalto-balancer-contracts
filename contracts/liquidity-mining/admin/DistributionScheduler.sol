...
import "../../interfaces/liquidity-mining/IRewardTokenDistributor.sol";
...
contract DistributionScheduler {
     ...
     * @param token - The token which is to be distributed among gauge depositors.
     */
    function getPendingRewards(IRewardTokenDistributor gauge, IERC20 token)
        public
        view
        returns (uint256)
    {
    ...
    function getPendingRewardsAt(
        ...
        mapping(uint32 => RewardNode) storage rewardsList = _rewardsLists[
            _getRewardsListId(gauge, token)
        ];
    ...
    function scheduleDistribution(
        ...
        require(
            rewardDistributor == address(this),
        ...
        require(
            ...
            "Distribution must start at the beginning of the week"
        );

        token.safeTransferFrom(msg.sender, address(this), amount);

        _insertReward(
            _rewardsLists[_getRewardsListId(gauge, token)],
            uint32(startTime),
            uint224(amount)
        );
    }
    ...
    function startDistributionForToken(IRewardTokenDistributor gauge, IERC20 token) public {
        ...
        (uint32 firstUnprocessedNodeKey, uint256 rewardAmount) = _getPendingRewards(
            rewardsList,
            block.timestamp
        );
    ...
    function _getRewardsListId(IRewardTokenDistributor gauge, IERC20 rewardToken)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _getPendingRewards(
        ...
        uint256 targetKey
    ) internal view returns (uint32, uint256) {

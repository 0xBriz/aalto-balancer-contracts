...
contract OracleWeightedPool is BaseWeightedPool, InvariantGrowthProtocolFees, PoolPriceOracle {
        ...
        if (miscData.oracleEnabled()) {
            ...
            if (oracleCurrentIndex != oracleUpdatedIndex) {
                _setMiscData(
                    ...
                    miscData.setOracleIndex(oracleUpdatedIndex).setOracleSampleCreationTimestamp(
                        block.timestamp
                    )
                );
            }
    ...
    function _cacheInvariantAndSupply() internal {
        ...
        if (miscData.oracleEnabled()) {
            _setMiscData(
                miscData
                    .setLogInvariant(LogCompression.toLowResLog(getLastInvariant()))
                    .setLogTotalSupply(LogCompression.toLowResLog(totalSupply()))
            );
        }
    }

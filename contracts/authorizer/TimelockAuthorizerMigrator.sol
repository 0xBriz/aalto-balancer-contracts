...
import "../interfaces/vault/IBasicAuthorizer.sol";
...
contract TimelockAuthorizerMigrator {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _rolesData.length; i++) {
            rolesData.push(
                OldRoleData(_rolesData[i].role, _rolesData[i].target)
            );
        }

        // Enqueue a root change execution in the new authorizer to set it to the desired root address
        rootChangeExecutionId = _newAuthorizer.scheduleRootChange(
            _root,
            _arr(address(this))
        );
    }
    ...
    function finalizeMigration() external {
        ...
        bytes32 setAuthorizerId = IAuthentication(address(vault)).getActionId(
            IVault.setAuthorizer.selector
        );
        bool canSetAuthorizer = oldAuthorizer.canPerform(
            setAuthorizerId,
            address(this),
            address(vault)
        );
    ...
    function _afterMigrate() internal {
        ...
        require(
            newAuthorizer.canExecute(rootChangeExecutionId),
            "CANNOT_TRIGGER_ROOT_CHANGE_YET"
        );
